...
On React 17, the closed field hydrates as a deterministic native shell before
the React Aria collection mounts after hydration. This avoids the upstream
hidden-collection mismatch while preserving the same interactive Tale API.
...
>
  {(p) => (
    <TagSelect.Item id={p.id} textValue={p.name}>
      {p.name}
    </TagSelect.Item>
  )}
</TagSelect.Root>
