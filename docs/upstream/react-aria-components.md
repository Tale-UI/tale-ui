...
| Meter and ProgressBar avoid `NaN` when `minValue === maxValue`                                                             | Adopted the upstream zero-percentage behavior in Tale-owned Indicator calculations and in ProgressCircle's custom percentage calculation. Connected all three Tale label parts to React Aria's accessible-name context without changing their span DOM/ref contract. | Shared `getPercentage` utility, Meter/ProgressBar/ProgressCircle implementations and tests, component guides |
